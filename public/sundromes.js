...
export const Sundromes = {
    packages: [
        { 
            ...
            price: 4, 
            stripeId: 'price_1TDRNXJAVbLLS5IpL2kral1o',
            year: 1991,
            desc: 'Κλήση σε διανομέα ή σερβιτόρο και ομαδικό chat.',
            ui_ids: ['chatWrapper', 'staffContainer', 'btnFakeLock', 'btnSettings', 'btnSettingsAdmin']
        },
        { 
            key: 'pack_manager', 
            name: '2. 👨‍🍳 Manager & Παραγγελιοληψία', 
            price: 10, 
            stripeId: 'price_1TDRNaJAVbLLS5Ip8prJmGrD',
            year: 1992,
            desc: 'Παραγγελιοληψία, Έξοδα, Στατιστικά (Tζίροι/Άτομο), Εκτυπωτές.',
            ui_ids: ['desktopArea', 'btnNewOrderSidebar', 'btnExpenses', 'btnMenuToggle', 'btnSettings', 'btnWallet', 'btnFakeLock', 'rowSwitchStaff', 'rowSwitchCust', 'rowStaffCharge', 'rowPrinterEnabled', 'btnSettingsStore', 'btnSettingsPrint', 'btnSettingsBot', 'rowTotalTables', 'btnSettingsAdmin'] // ✅ Staff & Charge controlled here
        },
        { 
            key: 'pack_delivery', 
            name: '3. 🛵 Delivery QR & Κρατήσεις', 
            price: 15, 
            stripeId: 'price_1TDRNYJAVbLLS5Ip2cDXB22R',
            year: 1993,
            desc: 'QR για παραγγελίες delivery και διαχείριση κρατήσεων.',
            ui_ids: ['desktopArea', 'resWrapper', 'btnShowLink', 'btnSettings', 'btnFakeLock', 'btnSettingsQr', 'btnMenuToggle', 'btnSettingsStore', 'rowSwitchCust', 'secSchedule', 'btnSettingsPrint', 'rowPrinterEnabled', 'rowReservations', 'btnSettingsAdmin'] // ✅ Added btnSettingsAdmin for Stripe
        },
        { 
            key: 'pack_tables', 
            name: '4. 🍽️ Παραγγελία Τραπεζιού', 
            price: 15, 
            stripeId: 'price_1TDRNbJAVbLLS5IpeKBVsVvh',
            year: 1994,
            desc: 'Δυνατότητα παραγγελίας από τον πελάτη στο τραπέζι.',
            ui_ids: ['desktopArea', 'btnModeTable', 'btnSettings', 'btnFakeLock', 'btnSettingsQr', 'btnQrTables', 'btnMenuToggle', 'btnSettingsStore', 'btnSettingsAdmin'] // ✅ Added btnSettingsAdmin for Stripe
        },
        { 
            key: 'pack_pos', 
            name: '5. 💳 POS & E-Invoicing', 
            price: 20, 
            stripeId: 'price_1TDRNYJAVbLLS5Ipnsrk6rhY',
            year: 1995,
            desc: 'Ηλ. Τιμολόγηση, Σύνδεση POS και SoftPOS στο κινητό.',
            ui_ids: ['desktopArea', 'btnCashRegister', 'softPosSettingsContainer', 'physicalPosSettingsContainer', 'btnSettings', 'btnFakeLock', 'btnSettingsAdmin', 'btnSettingsEinvoicing'] // ✅ Added physicalPosSettingsContainer
        },
        { 
            key: 'pack_loyalty', 
            name: '6. 🎁 Επιβράβευση (Loyalty)', 
            price: 5, 
            stripeId: 'price_1TDRNYJAVbLLS5Ippmnf0Qzw',
            year: 1996,
    ...
    hasAccess: (user, key) => {
        ...
        const match = storeEmail.match(/(199[1-8])/);
        if (match) {
            ...
            // ✅ 1997: ΠΡΟΣΦΟΡΑ 10€ - Όλα ενεργά ΕΚΤΟΣ από το POS
            if (year === 1997) return key !== 'pack_pos';

            const pkg = Sundromes.packages.find(p => p.key === key);
            // ✅ Cumulative Matching: Όλα τα προηγούμενα πακέτα είναι ενεργά
            if (pkg && pkg.year <= year) return true;
        }
